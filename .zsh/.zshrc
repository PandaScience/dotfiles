...
widget-fzf_cd_to_path() {
  local pth
  pth=$(fd --type d --hidden --exclude .git | fzf --preview 'tree -C')
  if [[ -n $pth ]]; then
    cd -- ${~pth}
    zle kill-buffer
    zle accept-line
...
local -a completion_tools=(kubectl minikube k3d kind istioctl kubeone argocd flux helm dlv kubebuilder kafkactl kyverno sbx stern)
local tool cache
